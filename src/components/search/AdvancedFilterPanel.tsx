...
export const AdvancedFilterPanel: React.FC<AdvancedFilterPanelProps> = ({
  ...
  const quranVerseNumbers = quranData
    ...
    .filter(Boolean);
  const quranMinVerse = Math.min(...quranVerseNumbers);
  const quranMaxVerse = Math.max(...quranVerseNumbers);

  // Hadith-specific unique values
  const hadithNumbers = hadithData.map((_, index) => index + 1);
  const hadithMinNumber = Math.min(...hadithNumbers);
  const hadithMaxNumber = Math.max(...hadithNumbers);
  ...
  const getActiveFilterCount = () => {
    ...
    if (filters.dataSources.length !== 3) count++;
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder="Min"
                  />
                  <span className="text-xs text-stone-500 self-center">to</span>
                  <input
                    type="number"
                    min={quranMinVerse}
                    max={quranMaxVerse}
                ...
                <div className="grid grid-cols-2 gap-2">
                  {uniqueQuranPlaces.map((place) => (
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={filters.quranPlaceOfRevelation.includes(place)}
                        onChange={() =>
